...
function staffDisplay(staffId,len) {

	for ( var i = 1; i <= len; i++) {
		if (staffId == i) {
			document.getElementById('staff_' + staffId).style.display = 'block';
			document.getElementById('staffTab_' + staffId).className = 'active';
		} else {
			document.getElementById('staff_' + i).style.display = 'none';
			document.getElementById('staffTab_' + i).className = 'noactive';
		}
	}
}
